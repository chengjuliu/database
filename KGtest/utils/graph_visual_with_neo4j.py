...
# 指定Parquet文件路径
# 首次更新
GRAPHRAG_FOLDER=r"C:\Code\Python_Code\LightRAGTest\GraphRAG\KGtest\output"
# 增量更新
# GRAPHRAG_FOLDER="/Users/janetjiang/Desktop/agi_code/LightRAGTest/GraphRAG/update_output"



# 数据库连接相关参数配置
NEO4J_URI="neo4j+s://1e1c22ca.databases.neo4j.io"
NEO4J_USERNAME="neo4j"
NEO4J_PASSWORD="v-4G2boQPqj-V7oeItqney-G8Xn1scGDVPwIigISO7M"
...
# 1、创建或更新documents
# 从指定的 Parquet 文件 create_final_documents.parquet 中读取 id、title 和 text 这三列
# 并将它们加载到一个名为 doc_df 的 Pandas 数据帧中。这个数据帧可以进一步用于数据处理、分析或导入操作
doc_df = pd.read_parquet(f'{GRAPHRAG_FOLDER}/create_final_documents.parquet', columns=["id", "title", "text"])
...
# SET e += value {.title, .type, .description, .human_readable_id, .id, .description_embedding, .text_unit_ids}从 value 对象中提取属性，并将它们赋值给节点 e 的同名属性
...
# SET rel += value {.combined_degree, .weight, .human_readable_id, .description, .text_unit_ids}将 value 对象中的 combined_degree、weight、human_readable_id、description 和 text_unit_ids 属性合并到该关系上
